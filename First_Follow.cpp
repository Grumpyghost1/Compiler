        ...
        while(it < productions[i].length()){
            if(productions[i][it] >= 'A' && productions[i][it] <= 'Z'){
                ...
                for(auto &value: first[productions[i][it]]){
                    if(value != 'e') first[nonTerminal].insert(value);
                    else eFound = true;
                }
               
                if(!eFound){
                    while(it < productions[i].length() && productions[i][it] != '|') ++it;
                    if(it != productions[i].length()) ++it;
                }
            ...
            while(productions[j].find(nonTerminal, idx) != string::npos){
                ...
                while(it < productions[j].length() && productions[j][it] != '|'){
                    ...
                    if(c >= 'A' && c <= 'Z'){
                        ...
                        if(follow[nonTerminal].find('e') == follow[nonTerminal].end()) break;
